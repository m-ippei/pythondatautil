...
class DataUtil:
    ...
    def __getTmpName(self, ext: str = ".txt") -> str:
        ...
        for v in os.listdir(os.getcwd()):
            re_obj_searched = re_obj.search(v)
            if re_obj_searched:
                tmp_num_list.append(int(re_obj2.sub("", re_obj_searched.group())))
    ...
    def __getFileNameHelper(self, order_file_name, ext=".txt"):
        ...
        if os.path.exists(os.path.dirname(file_name)):
            if order_ext:
                if ext != order_ext:
                    raise ValueError(
                        f"有効な拡張子ではありません。{os.path.basename(file_name)}"
                    )
            else:
        ...
        if content_list_len > 0:
            ...
            if list_in_list_len == content_list_len:
                ...
                for index, v in enumerate(content_list, start=1):
                    if item_len != len(v):
                        raise ValueError(
                            f"一致しないデータ数 {item_len_index}行目:{item_len}個,{index}行目:{len(v)}個"
                        )
                ...
                if list_in_list_len > 0:
                    raise ValueError(
                        "list型の中にList型とそれ以外を混在させることできません。"
                    )
    ...
    def r_auto(self, path):
        ...
        if ext in [".txt", ".csv", ".tsv"]:
            ...
            if isinstance(content_list[0], list):
                ...
                if len(
                    [len(v) for v in content_list if len(v) == 1 or len(v) == 0]
                ) == len(content_list):
                    content_list2 = []
    ...
    def w_csv(self, content_list, filename="", write_encoding="utf-8"):
        ...
        """
        with open(
            self.__getFileNameHelper(filename),
            mode="w",
